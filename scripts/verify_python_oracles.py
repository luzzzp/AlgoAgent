...
def main() -> None:
    ...
    for bundle in load_problems(args.problems):
        total += 1
        updated, ok, has_solution = verify_bundle(bundle, executor, args.max_solutions_per_problem)
    ...
    report = {"stage": "verify_python_oracles", "total": total, "verified": verified, "failed": failed, "no_solution": no_solution}
...
def verify_bundle(
    ...
    return ProblemBundle(spec=bundle.spec, tests=bundle.tests, oracle=oracle), ok, has_solution
